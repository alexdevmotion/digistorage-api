...
class DigiStorageApi:
    ...
    def __init_token(email, password):
        ...
        if r.status_code != 200:
            raise DigiStorageApiException('Error authenticating. Wrong username/password?')
    ...
    def __get_mount_by_name(self, mount_name):
        ...
        if not found_mounts_with_name:
            raise DigiStorageApiException('No mount has been found under the name: ' + mount_name)
    ...
    def upload_file(self, file_path, remote_path, remote_file_name=None):
        ...
        with open(file_path, 'rb') as f:
            ...
            f.close()
            if r.status_code != 200:
                raise DigiStorageApiException('Error uploading file: ' + file_path)
            return r
    ...
    def create_folder(self, remote_path):
        ...
        if r.status_code != 200:
            raise DigiStorageApiException('Error creating folder: ' + remote_path)
        return r
    ...
    def remove_file_folder(self, remote_path):
        ...
        if r.status_code != 200:
            raise DigiStorageApiException('Error removing file/folder: ' + remote_path)
        return r
    ...
    def destroy(self):
        """ Make sure to call this method when finished with interacting w/ the API to revoke the auth token """
        r = self.session.delete(API_BASE + '/token', headers={'Accept': '*/*'})
        if r.status_code != 204:
            raise DigiStorageApiException('Error revoking access token')
        return r
...
if __name__ == '__main__':
    ...
    except DigiStorageApiException as dsae:
        raise dsae
    finally:
        dsa.destroy()
